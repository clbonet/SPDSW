...
sys.path.append("../../lib")
from swspd import sliced_wasserstein_spd, emd1D
...
class Embedding:
    ...
    def __init__(self, shape_X, num_projections, device, dtype, random_state, sampling="sym"):

        self.A = self.generate_projections(
            shape_X, num_projections, device, dtype, random_state, sampling
        )
    ...
    def generate_projections(self, shape_X, num_projections,
        ...
        if sampling == "sym":
            ...
            A = rng.normal(size=(num_projections, shape_X, shape_X))
            A = torch.tensor(
                A,
                dtype=dtype,
                device=device
            ...
            A /= torch.norm(A, dim=(1, 2), keepdim=True)  

        return A
    
    
    def sliced_cost_spd(self, Xs, Xt, u_weights=None, v_weights=None, p=2):
        n, _, _ = Xs.shape
        m, _, _ = Xt.shape
    ...
    def sliced_wasserstein_spd(self, Xs, Xt, p=2, 
        ...
        return self.sliced_cost_spd(Xs, Xt, u_weights=u_weights, v_weights=v_weights, p=p)
...
#         projs = get_projs(num_projs, d, device, torch.float64)
        sw_class = Embedding(d, num_projs, device, torch.float64, 42)
...
#                             sw = sliced_wasserstein_spd2(zs[:,0,l], cov_Xt[:,0,l], num_projs, device, p=2)
                            sw = sw_class.sliced_wasserstein_spd(zs[:,0,l], cov_Xt[:,0,l], p=2)
                        elif args.loss == "lew":
