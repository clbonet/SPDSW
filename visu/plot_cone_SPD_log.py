...
ts = torch.linspace(-buseman_coord.max()-0.1,-buseman_coord.min()+0.1,100)
geod = []
...
cone_mesh1 = Plane(pos=(height/2, height/2, 0), normal=(0, 0, 1), s=(height, height), alpha=.9, res=(res, res))
cone_points1= cone_mesh1.points()
cone_mesh2 = Plane(pos=(height/2, height/2, 0), normal=(0, 0, 1), s=(height, height), alpha=.9, res=(res, res))
...
cone = merge(cone_mesh1,cone_mesh2).color('grey').alpha(.4)
...
geodesic = Line(mat2point(geod)).color('black').lw(4)
...
for i in range(len(vect_B0)):
    ...
    tab_proj.append(Line(mat2point(geod_le.numpy())).color('blue').lw(2))
...
s_proj = Spheres(proj_mat, r=.06).c("green")
...
plt.at(1).show(cone,cone.silhouette().linewidth(3).color("grey"), geodesic,tab_proj, s,s_proj, "Geodesic projections", zoom=1.1, interactive=0)
plt.at(0).show(cone,cone.silhouette().linewidth(3).color("grey"), geod_curves, "Random geodesics", zoom=1.1, interactive=1)
