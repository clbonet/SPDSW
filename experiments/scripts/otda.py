...
RESULTS = os.path.join(EXPERIMENTS, "results/otda.csv")
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
DTYPE = torch.float64
RNG = np.random.default_rng(SEED)
mem = Memory(
    location=os.path.join(EXPERIMENTS, "scripts/tmp_otda/"),
    verbose=0
)
...
#         "multifreq": [False]
    ...
    if args.task == "session":
        ...
        RESULTS = os.path.join(EXPERIMENTS, "results/otda_cross_session.csv")
    elif args.task == "subject":
        hyperparams["cross_subject"] = [True]
        RESULTS = os.path.join(EXPERIMENTS, "results/otda_cross_subject.csv")
