...
class Riemannian_Model:
    ...
    # 		self.filter_bank = load_filterbank(self.bw,self.fs,order=self.forder,max_freq=40,ftype = self.ftype) # get filterbank coeffs 
        ...
        self.fs = 250 # Sampling frequency
        self.bw = [25] ## bandwidth
        self.forder = 8
        max_freq = 30
        self.ftype = "butter"

        self.filter_bank = load_filterbank(bandwidth = self.bw, fs = self.fs, order = self.forder, 
                                           max_freq = max_freq, ftype = self.ftype)
    ...
    def run_riemannian(self):
        ...
        #print(self.train_data.shape, train_feat.shape)
